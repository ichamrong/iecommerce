...
/**
 * Saga Listener for the Inventory module. Reacts to Order events published via the Outbox pattern.
 *
 * <p><b>Banking/Insurance standard:</b> This ensures that if the Order module successfully commits,
 * the Inventory operations will follow eventually (at-least-once). If inventory fails (e.g.
 * OutOfStock), a compensating transaction should be triggered (not implemented yet, would involve
...
public class InventorySagaListener {
  ...
  public void onOrderConfirmed(OrderConfirmedEvent event) {
    log.info(
        "Saga [Inventory]: Reserving stock for order {} (tenant: {})",
        event.orderId(),
        event.tenantId());
    ...
    try {
          ...
          .forEach(
              item ->
                  inventoryService.reserveStock(
                      event.tenantId(), item.productVariantId(), item.quantity()));
      ...
      log.error(
          "Saga [Inventory]: Failed to reserve stock for order {}. Emitting failure event.",
          event.orderId(),
          ex);
      inventoryService.saveOutbox(
          event.tenantId(),
          "InventoryOperationFailedEvent",
  ...
  public void onOrderCancelled(OrderCancelledEvent event) {
    log.info(
        "Saga [Inventory]: Releasing stock for order {} (tenant: {})",
        event.orderId(),
        event.tenantId());
  ...
  public void onOrderShipped(OrderShippedEvent event) {
    log.info(
        "Saga [Inventory]: Deducting stock permanently for order {} (tenant: {})",
        event.orderId(),
        event.tenantId());
